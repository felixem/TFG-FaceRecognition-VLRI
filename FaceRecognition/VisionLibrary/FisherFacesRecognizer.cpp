...
namespace tfg
{
	...
	void FisherFacesRecognizer::saveReconstruction(const cv::Mat& img, const std::string &directory, const std::string& baseName)
	{
		...
		for (int num_component = 0; num_component < std::min(16, W.cols); num_component++) {
			...
			imwrite(cv::format(("%s/" + baseName + "_%d.png").c_str(), directory.c_str(), num_component), reconstruction);
		}
	}
	...
	void FisherFacesRecognizer::load(const std::string& fichero)
	{
		this->model->load(fichero);
	}

	//Guardar en fichero el modelo
	void FisherFacesRecognizer::save(const std::string& fichero)
	{
